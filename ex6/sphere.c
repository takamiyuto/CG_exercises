...
#define __SP_C_MAX_LENGTH__ 1000
        GLchar errorMessage[__SP_C_MAX_LENGTH__];
        glGetShaderInfoLog(shaderHandle, 1000, NULL, errorMessage);
        fprintf(stderr, errorMessage);
        fprintf(stderr, "\n");
#undef __SP_C_MAX_LENGTH__
...
#define __SP_C_MAX_LENGTH__ 1000
        GLchar errorMessage[__SP_C_MAX_LENGTH__];
        glGetProgramInfoLog(program, __SP_C_MAX_LENGTH__, NULL, errorMessage);
        fprintf(stderr, errorMessage);
        fprintf(stderr, "\n");
#undef __SP_C_MAX_LENGTH__
...
static void LoadShaders(char* vs_name, char* fs_name) {
    //g_program = BuildProgram("noise.vs.glsl", "noise.fs.glsl");
    g_program = BuildProgram(vs_name, fs_name);
...
static void InitGL(char* vs_name, char* fs_name) {
    glClearColor(1.0, 1.0, 1.0, 1.0);

    BuildGeometry();
    LoadShaders(vs_name, fs_name);
...
static void usage(char* prog_name) {
    ...
    printf("Usage:\n");
    printf("%s vertex_shader.vs frag_shader.fs\n", prog_name);
}
